...
class Nota extends CI_Controller
{
  ...
  function pembayaran(){ 
      ...
      if($cek->num_rows() == 1){
        ...
        $allBayar = $this->data_model->get_byid('a_nota_bayar', ['id_cus'=>$idcus]);
        $data = array(
  ...
  function prosessimpan(){
      ...
      for ($i=0; $i < $con; $i++) { 
          $dtlist = [
            ...
            'total_harga' => round($totalharga[$i],2),
            'tgl_nota' => date('Y-m-d'),
  ...
  function editharga(){
      ...
      $this->data_model->updatedata('id_nota',$id,'a_nota',['harga_satuan'=>$hrg,'total_harga'=>round($newharga,2)]);
          ...
          for ($i=0; $i <count($panjang) ; $i++) { 
              if($panjang[$i]!="" AND $harga[$i]!=""){
                  if($i==0){
                    ...
                    $this->data_model->updatedata('id_nota',$id_nota,'a_nota',['total_panjang'=>$panjang[$i], 'harga_satuan'=>round($harga[$i],2), 'total_harga'=>round($total_harga,2)]);
                  } else {
                    $total_harga = floatval($harga[$i]) * floatval($panjang[$i]);
                    $dtlisr = [
                      'no_sj' => $nosj,
  ...
  function del_bayar(){
      ...
      $this->data_model->delete('a_nota_bayar','id_pemb',$id_pemb);
