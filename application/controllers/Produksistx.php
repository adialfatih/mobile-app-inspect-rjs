...
class Produksistx extends CI_Controller
{
  ...
  function prosesupdate(){
        ...
        $dtlist = [
            ...
            'tanggal' => $tgl,
            'operator' => $operator,
            'bp_can_join' => $in_ukuran<50 ? 'true':'false',
            'dep' => $dep,
            'loc_now' => $depnow,
            'yg_input' => 0,
            'kode_upload' => 'tes'
  ...
  function delfolgrey(){
        ...
        $_igukuran = $this->data_model->get_byid('data_ig',['kode_roll'=>$kode_roll])->row("ukuran_ori"); 
        ...
        if($jnsfol == "Grey"){
            $this->data_model->saved('data_igtujuan',['kode_roll'=>$kode_roll,'konstruksi'=>$kons,'ukuran'=>$_igukuran,'tujuan_proses'=>'Grey']);
