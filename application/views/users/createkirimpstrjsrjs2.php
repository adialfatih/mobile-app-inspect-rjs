...
<body>
    ...
    <h1>Kode Paket <?=$kd;?></h1>
    <small class="sm"><?=$newToday;?>, <strong><?=$prinTgl;?></strong>, Username : <strong id="nmoptid">Nur Hikmah</strong></small><br><small class="sm">Konstruksi : <strong><?=$kons2;?></strong></small>
    ...
    <?php
        if($siap_jual=="n"){
            ...
            $kdrol = $this->db->query("SELECT kode_roll FROM data_fol WHERE konstruksi='$kons' AND posisi='Pusatex'");
        }
        $ar_kdrol = array();
                ...
                input: {
                    selection: (event) => {
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            data: {"selection" : selection, "kons" : kons, "pkg" : pkg, "tipee":"0", "siap_jual":siap_jual},
                            cache: false,
