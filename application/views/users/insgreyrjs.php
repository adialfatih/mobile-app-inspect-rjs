...
<body>
    ...
    <div class="container">
        ...
        <div class="form-label">
            ...
            <input type="tel" class="ipt" name="ukuran" id="ukuran" placeholder="Input Ukuran" >
            <small class="mtr">Meter</small>
        </div>
        <!-- <div class="form-label">
        ...
        <div class="box-masket">
            <label for="masket">Data BS</label>
            <div class="form-label">
                <span for="ukuranbs">Ukuran BS (Mtr)</span>
                <input type="tel" class="ipt" name="bs" id="bs" value="0" placeholder="Input BS" >
            </div>
            <div class="form-label">
                <span for="masket">Masukan Keterangan :</span>
            </div>
            <div class="form-ukuran">
                <textarea style="width:100%;height:70px;" class="ipt" id="masket" name="masket" placeholder="Masukan Keterangan BS Kenapa?"></textarea>
            </div>
        </div>
        <div class="box-masket" style="margin-top:20px;">
            <label for="masket">Inspect Ulang ? &nbsp;</label>
            <div class="form-label">
                <span for="ukuranbs">Kode Awal</span>
                <input type="text" class="ipt" name="kdlama" id="kdlama" oninput="lihatKodeini(this.value)" value="0" placeholder="Masukan Kode Lama" style="text-transform: uppercase;">
            </div>
            <div style="width:100%;background:#c9c9c9;border-radius:2px;margin-bottom:10px;font-size:13px;" id="hasilKodeini"></div>
            <div style="width:100%;background:#ccc;padding:5px;border-radius:2px;font-size:13px;">
                Kode awal di isi dengan kode yang anda inspect ulang. <strong style='color:#e81c0c;'>Hanya berlaku jika ada inspect ulang di Pusatex.</strong> Isi dengan angka nol jika bukan proses inspect ulang.
            </div>
        </div>
        
        <!-- <div class="form-label">
            <label for="bp">BP (21 s/d 49 Mtr)</label>
            <input type="tel" class="ipt" name="bp" id="bp" placeholder="UKURAN BP 21 s/d 49 Mtr" value="0" readonly>
        </div>
        <div class="form-label">
            <label for="bp">BC (6 s/d 20 Mtr)</label>
            <input type="tel" class="ipt" name="bc" id="bc" placeholder="UKURAN Bc 6 s/d 20 Mtr" value="0" readonly>
        </div> -->
    ...
    <script>
        ...
        function lihatKodeini(kode){
            if(kode=="0" || kode=="" || kode==0){
                $('#hasilKodeini').html('').css('padding', '0px');
            } else {
                $('#hasilKodeini').html('Loading...').css('padding', '5px');
                $.ajax({
                    url:"<?=base_url();?>usersrjs/carikodeig",
                    type: "POST",
                    data: {"kode" : kode},
                    cache: false,
                    success: function(dataResult){
                        ...
                        if(dataResult.statusCode==200){
                            var txt = 'Kode Roll : <strong>'+dataResult.kode_roll+'</strong><br>Kode Konstruksi : <strong>'+dataResult.konstruksi+'</strong><br>Nomor Mesin : <strong>'+dataResult.no_mesin+'</strong><br>Ukuran : <strong>'+dataResult.ukuran_ori+'</strong><br><font style="color:red;">Kode Roll ini akan di hapus ketika anda menyimpan.</font>';
                            $('#hasilKodeini').html(txt).css('padding', '5px');
                        } else {
                            $('#hasilKodeini').html(dataResult.psn).css('padding', '5px');
                        }
                    }
                });
            }
        ...
        function simpanData(){
            ...
            var kdlama = document.getElementById('kdlama').value;
            if(parseInt(idpotid) >= 0){
                
            if(kons!="" && mc!="" && beam!="" && okasd!="" && ori!="" && bs!="" && username!="null"){
                $.ajax({
                    url:"<?=base_url();?>usersrjs/prosesInsGrey",
                    type: "POST",
                    data: {"kons" : kons, "mc" : mc, "beam" : beam, "oka" : okasd, "ori" : ori, "bs":bs, "tgl":tgl, "username":username, "fromData":fromData, "idpotid":idpotid, "masket":masket, "kdlama":kdlama},
                        cache: false,
                        success: function(dataResult){
                            var dataResult = JSON.parse(dataResult);
                            if(dataResult.statusCode==200){
                                ...
                                loadData();
                                var koderollid = dataResult.psn;
                                if(parseInt(idpotid) > 0){
                                insertToApi(idpotid,koderollid,kons,ori,bs,'0',tgl,username); }
            ...
            function cekpotongan(){
                ...
                if(id == ""){
                    ...
                    $.ajax({
                        url: 'https://ajl.rdgjt.com/api/rindang',
                        type: 'GET',
                        dataType: 'json',
                        data: {"id" : id},
                        success: function(response) {
                            ...
                            //$('#autoComplete').val('' + konstruksi).prop('readonly', true);
                            $('#mc').val(''+nomesin);
                            //$('#mc').val('' + nomesin).prop('readonly', true);
                            $('#beam').val(''+nobeam);
                            //$('#beam').val('' + nobeam).prop('readonly', true);
                            $('#idoka').val(''+oka);
                            //$('#idoka').val('' + oka).prop('readonly', true);
                            setTimeout(function(){
                                $('#loadpot').html('<div class="submitid" onclick="cekpotongan()">Cari</div>');
                            }, 1000);
                            $('#testhasil').html('<span class="tytle">Informasi Potongan</span><div>Kode Potongan : <strong>'+id+'</strong> <font style="color:green;font-weight:bold;">Ditemukan</font></div><div>Tanggal Potong : <strong>'+formatwk+'</strong></div><div>Ukuran Potong : <strong>'+mtr+'</strong></div><div>Operator : <strong>'+op+'</strong></div>');
                            $('#testhasil').show();
                            $('#fromData').val('1');
                        },
        ...
        function insertToApi(idpotid,koderollid,kons,ori,bs,bp,tgl,username){
            $.ajax({
                    url:"https://ajl.rdgjt.com/api/rindang",
                    type: "POST",
                    data: {"kode" : idpotid, "ori":ori, "bs":bs, "bp":bp, "operator":username, "koderoll":koderollid, "posisi":"RJS", "tgl":tgl},
                    cache: false,
                    success: function(dataResult){
                        var dataResult = JSON.parse(dataResult);
                        if(dataResult.statusCode==200){
                            suksestoast('Success add to API');
                        } else {
                            peringatan('Erorr add to API');
                        }
                    }
            });
        }
        function deleteApiKode(koderoll){
            $.ajax({
                    url:"https://ajl.rdgjt.com/api/rindang",
                    type: "DELETE",
                    data: {"koderoll" : koderoll, "posisi":"RJS", "id":koderoll},
                    cache: false,
                    success: function(dataResult){
                        var dataResult = JSON.parse(dataResult);
                        if(dataResult.statusCode==200){
                            suksestoast('Success delete to API');
                        } else {
                            peringatan('Erorr delete to API');
                        }
                    }
            });
        }
