...
<body>
    ...
    <div class="container">
        <div class="form-label" id="khususAdmin" style="display:none;">
            ...
            <input type="date" name="oltgl" class="ipt" id="oldTanggalFolding" onchange="loadData2(this.value)">
        </div>
        ...
        <div class="form-label">
            <label for="autoComplete2">Folding</label>
            <div class="autoComplete_wrapper" style="width:49%;background:#ccc;">
                <select name="jnsFold" id="jnsFold" style="width:100%;padding:7px;" onchange="teschange()">
        ...
        <div class="form-label">
            ...
            <div class="autoComplete_wrapper">
                <input id="autoComplete" onchange="teschange()" type="search" dir="ltr" spellcheck=false autocorrect="off" autocomplete="off" autocapitalize="off">
            </div>
        ...
        <button class="btn-save" onclick="joinPieces2()">Join Pieces</button>
        <a href="" id="btnSelesai" class="btn-save2">Selesai</a>
    </div>
    <?php
        $ig = $this->db->query("SELECT * FROM data_ig WHERE kode_roll NOT IN (SELECT kode_roll FROM data_fol) ");
        $if = $this->db->query("SELECT * FROM data_if WHERE kode_roll NOT IN (SELECT kode_roll FROM data_fol) ");
        $ar_ig = array();
        foreach($ig->result() as $val){
            $ar_ig[] = '"'.$val->kode_roll.'"';
        }
        foreach($if->result() as $vali){
            $txt = '"'.$vali->kode_roll.'"';
            if(in_array($txt, $ar_ig)){} else {
            $ar_ig[] = '"'.$vali->kode_roll.'"'; }
        }
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function(dataResult){
                        ...
                        if(dataResult.statusCode==200){
                            ...
                            $('#resultKode').html("<span style='color:red;'>"+dataResult.psn+"</span>");
                        }
                    }
                });
            } else {
        ...
        function joinPieces2(){
            window.location.href = "<?=base_url('users/joinpieces/');?>"+personName;
        }
        if(personName == "septi" || personName == "syafiq"){
            $('#khususAdmin').show();
            $('#khususAdmin2').show();
            $('#khususAdmin3').show();
        ...
        document.getElementById('btnSelesai').href = "<?=base_url();?>usersrjs";
        function loadData2(tgl){
            loadData(tgl);
        }
        function loadData(tgl){
            $.ajax({
                ...
                type: "POST",
                data: {"username" : personName, "proses" : "folgrey", "tgl":tgl},
                cache: false,
                success: function(dataResult){
        ...
        function simpanData(){
            ...
            if(koderoll!="" && username!="null" && jnsFold!=""){
                $.ajax({
                    url:"<?=base_url();?>users2/prosesFolGrey",
                    type: "POST",
                    data: {"koderoll" : koderoll, "ori" : ukuranArray, "tgl":tgl, "username":username, "old_tgl" : old_tgl, "old_opt" : old_opt, "old_note" : old_note, "jnsFold":jnsFold},
                        cache: false,
                        success: function(dataResult){
                            var dataResult = JSON.parse(dataResult);
                            if(dataResult.statusCode==200){
