...
<body>
    ...
    <div class="container">
        ...
        <div class="form-label">
            <label for="jnsFold">Folding</label>
            <select name="jnsFold" id="jnsFold" style="width:48%;padding:5px;">
        ...
        <div class="form-label" style="margin-top:10px;">
            <label for="ukuran">Ukuran Meter</label>
        </div>
        <div class="form-ukuran">
            <input type="tel" id="ukuranJoin" name="ukuran" placeholder="Ukuran Meter">
            <small class="geser-yard">Meter</small>
        </div>
        
        <div class="form-ukuran">
            <input type="tel" id="ukuranJoin2" name="ukuran2" placeholder="Ukuran Yard">
        ...
        <div class="kolom2">
            ...
            <a href="<?=base_url();?>usersrjs/folding" id="btnJoin5" class="btn-save3">&laquo; Kembali</a>
        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const meterInput = document.getElementById("ukuranJoin");
            const yardInput = document.getElementById("ukuranJoin2");

            const meterToYard = (meter) => meter / 0.9144;
            const yardToMeter = (yard) => yard * 0.9144;

            meterInput.addEventListener("input", function () {
                if (this.value !== "") {
                    yardInput.value = (meterToYard(parseFloat(this.value))).toFixed(2);
                } else {
                    yardInput.value = "";
                }
            });
            ...
            yardInput.addEventListener("input", function () {
                if (this.value !== "") {
                    meterInput.value = (yardToMeter(parseFloat(this.value))).toFixed(2);
                } else {
                    meterInput.value = "";
                }
            });
            ...
            $( "#addButon50" ).on( "click", function() {
                ...
                if(inOllukr!=""){
                    $.ajax({
                        ...
                        type: "POST",
                        data: {"inOldkode" : inOldkode, "kons":kons, "inOllukr":inOllukr, "jnsFold":jnsFold},
                        cache: false,
        ...
        function simpanData(){
            ...
            var ukuranJoin2 = document.getElementById('ukuranJoin2').value;
            ...
            if(kons12!="" && username!="null"){
                $.ajax({
                    ...
                    type: "POST",
                    data: {"koder" : koder1, "ukra" : ukra, "st":st1, "username":username, "tgl":tgl, "kons":kons12, "ukuranJoin":ukuranJoin,"ukuranJoin2":ukuranJoin2, "jnsFold":jnsFold},
                        cache: false,
                        success: function(dataResult){
                            var dataResult = JSON.parse(dataResult);
                            if(dataResult.statusCode==200){
