...
<body style="background: #ebe8e8;">
    ...
    <div class="container">
        ...
        <a href="javascript:void(0);" onclick="tesToFinishing()" id="btnToFinishing" class="btn-save2" style="background:#d47706;border:none;">Terima Barang Dari Finishing</a>
        <a href="" id="btnSelesai" class="btn-save2">Selesai</a>
        
        <div class="box-masket" id="tapilanReady">
            <label class="lbl" for="tapilanReady">Ready to Inspect Finish</label>
            <div class="fortable">
                <table id="fortable56" style="font-size:12px;">
                    <tr>
                        <td>Loading...</td>
                    </tr>
                </table>
    ...
    <?php
        //$ig = $this->db->query("SELECT kode_roll,loc_now FROM data_ig WHERE loc_now IN ('Pusatex','iff') AND kode_roll NOT IN (SELECT kode_roll FROM data_if)");
        $ig = $this->db->query("SELECT * FROM data_if_before WHERE status='ready'");
        $ar_ig = array();
    ...
    <script>
        function onsalin(){
            let textInput = document.getElementById("teksToCopy");
            textInput.select();
            textInput.setSelectionRange(0, 99999); // Untuk mendukung perangkat seluler
            navigator.clipboard.writeText(textInput.value)
                .then(() => {
                    suksestoast('Teks berhasil disalin.');
                })
                .catch(err => {
                    peringatan('Teks Gagal Disalin');
                });
        };
        function showReadyToInsFinish(kons){
            $.ajax({
                url:"<?=base_url();?>users/showReadyToInsFinish",
                type: "POST",
                data: {"kons" : kons},
                cache: false,
                success: function(dataResult){
                    $('#fortable56').html(dataResult);
                }
            });
        }
        showReadyToInsFinish('all');
        ...
        function tesToFinishing(){
            if(personName == "tofan" || personName == "nifan" || personName == "septi"){
                //document.getElementById('btnToFinishing').href = "<=base_url('usersrjs/ininsfinish');?>";
                document.location.href = "<?=base_url('usersrjs/ininsfinish');?>";
            } else {
                let now = new Date();
                ...
                if (hour >= 8 && hour < 16) {
                    Swal.fire('Maaf, halaman ini hanya bisa diakses oleh tofan dan nifan antara pukul 08:00 - 16:00.');
                } else {
        ...
        function delReadyToInsFinish(id,kons){
            var result = window.confirm("Apakah Anda yakin ingin menghapus?");
            if (result) {
                $.ajax({
                    url:"<?=base_url();?>users/delbefore2",
                    type: "POST",
                    data: {"id" : id},
                    cache: false,
                    success: function(dataResult){
                        var dataResult = JSON.parse(dataResult);
                        if(dataResult.statusCode==200){
                            peringatan('Hapus kode '+id+'');
                            showReadyToInsFinish(kons);
                        } 
                    }
                });
            } else { console.log('Erorr'); }
                    ...
                    data: {"koderoll" : koderoll, "ori" : ukuranArray, "bs":ukuranBSArray, "bp":ukuranBPArray, "crt":crt, "tgl":tgl, "username":username, "ukrSebelum":ukrSebelum, "masket":masketArray},
                        ...
                        success: function(dataResult){
                            ...
                            if(dataResult.statusCode==200){
                                ...
                                for (let po = 1; po < 11; po++) {$('#boxnewBS'+po+'').hide();}

                                document.getElementById('bs1').value = '0';
                                document.getElementById('bp1').value = '0';
