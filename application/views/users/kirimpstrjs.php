...
<body>
    ...
    <div class="container">
        ...
        <div class="kotaknewpkg">
            <span style="color:red;font-weight:bold;font-size:16px;">Join Grey</span>
            <div style="width:100%;display:flex;flex-direction:column;">
                <label for="joinGrey" style="color:2e2e2e;">Masukan Kode Roll untuk Join Grey</label>
                <input type="text" placeholder="BR12,BR13" id="joinGrey" style="padding:7px;outline:none;border:1px solid #ccc;margin-top:5px;border-radius:3px;">
                <p style="margin:10px 0;font-size:15px;">Note : Pastikan kode yang di akan di join <strong style="color:red;">belum berada di packinglist</strong>. Ukuran joing akan otomatis di hitung.</p>
                <button style="margin-top:10px;outline:none;border:none;padding:7px;background:#4184f0;color:#fff;border-radius:3px;" onclick="joinpkGrey()">Join Kode Roll</button>
            </div>
            
        </div>

        <button class="btn-save" style="background:#575757;border:1px solid #FFF;" id="btn-upstok">Edit Roll</button>
        <button class="btn-save" style="background:#575757;border:1px solid #FFF;" id="btn-receive">Terima Barang Dari Rindang</button>
    ...
    <script>
        ...
        function joinpkGrey(){
            var input = document.getElementById('joinGrey').value;
            if(input!=""){
                if (/\s/.test(input)) {
                    Swal.fire('Gagal','Tidak boleh ada spasi','error');
                    return;
                }
                // Cek apakah ada koma di akhir
                if (input.endsWith(",")) {
                    Swal.fire('Gagal','Tidak boleh ada koma di terakhir kode','error');
                    return;
                }
                ...
                // Cek jumlah kode
                if (codes.length < 2) {
                    Swal.fire('Gagal','Minimal harus ada dua kode yang dipisahkan koma','error');
                    return;
                }
                // Cek setiap kode harus kombinasi huruf dan angka
                const validFormat = /^[A-Z0-9]+$/i;
                for (const code of codes) {
                    if (!validFormat.test(code)) {
                    Swal.fire('Gagal','Setiap kode hanya boleh berisi huruf dan angka (tanpa karakter khusus).','error');
                    return;
                    }
                }
                $.ajax({
                    url:"<?=base_url();?>usersrjs/joinKodeGrey",
                    type: "POST",
                    data: {"username" : personName, "txt" : input},
                    cache: false,
                    success: function(dataResult){
                        var dataResult = JSON.parse(dataResult);
                        if(dataResult.statusCode==200){
                            Swal.fire('Berhasil',dataResult.psn,'success');
                            document.getElementById('joinGrey').value='';
                        } else {
                ...
                Swal.fire('Gagal','Isi Kode Roll Dengan Benar','error');
            }
        }
        loadData();
        ...
        $( "#btn-receive" ).on( "click", function() {
            window.location.href = "<?=base_url('usersrjs/terimabarang');?>";
        });
                ...
                $.ajax({
                    ...
                    data: {"kodekons" : kodekons, "tgl":tgl, "username":username, "opt":namaopt},
                        ...
                        success: function(dataResult){
                            ...
                            if(dataResult.statusCode==200){
                                setTimeout(() => {
                                    window.location.href = "<?=base_url('usersrjs/createkirimpst2/');?>"+dataResult.psn+"";
                                }, "500");
                            }
                        }
                });
